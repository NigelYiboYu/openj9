 ...
 * \brief
 *
 * Use vector instructions to find the index of a sub-string inside a string assuming both strings have the same element size.
 * Each element is 1-byte for compact strings and 2-bytes for non-compressed strings.
 *
 * \details
 *
 * The vector sequence searches for the first character of the sub-string inside the source/master string. If the first character
 * is located, it'll perform iterative vector binary compares to match the rest of the sub-string starting from the first character position.
 *
 ...
 * \param isCompressed true if the string is compressed.
 *
 * \return a register for that contains the indexOf() result.
*/
TR::Register*
inlineVectorizedStringIndexOf(TR::Node* node, TR::CodeGenerator* cg, bool isCompressed)
   {
   // element size is 2-byte if both s1 and s2 are decompressed strings and it is 12 otherwise.
   // 0 for byte, 1 for halfword
   const uint32_t elementSizeMask = isCompressed ? 0 : 1;
   const bool isUTF16 = !isCompressed;
   ...
   if (comp->getOption(TR_TraceCG))
      traceMsg(comp, "inlineVectorizedStringIndexOf. is compressed %d\n", isCompressed);

   // Get call parameters
   // s1Value and s2Value are byte arrays
   // This evaluator function handles different indexOf() instrinsics, some of which are static calls without a receiver. Hence, the need for static call check.
   ...
   // Start
   TR::Instruction* cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, labelStart);
   ...
   generateRRRInstruction(cg, TR::InstOpCode::getAddThreeRegOpCode(), node, tmpReg, s1ValueReg, s1VecStartIndexReg);
   generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, loadLenReg, -1);                // [load_index = load_len - 1]
   ...
   cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, labelDone, regDeps);
